...
**Nom i cognoms:** Roman Zyuzin Alexandrov

**Data:** 23/03/2026  **Grup:** 2DAM
...
- [x] B) Perquè `await` dins `initState()` bloqueja el fil de la UI i Flutter llança un error
- [ ] C) Perquè `initState()` ha de retornar síncronament perquè el framework pugui cridar `build()` immediatament després i construir la UI
...
- [x] A) Perquè `initialize()` sol·licita el permís de càmera a l'usuari; sense cridar-lo, Flutter no demanarà permís mai
...
- [ ] C) Perquè `CameraPreview` necessita que el controlador hagi completat la inicialització amb el sensor de la càmera per poder mostrar el flux de vídeo; si no s'ha inicialitzat, llança una excepció
...
- [x] A) Copiar directament el fitxer a la carpeta `/storage/emulated/0/DCIM/` usant `File.copy()` sense permisos addicionals
